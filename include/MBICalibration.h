...
extern "C"
{
	...
	*/
	...
	public:
        ...
        /// @brief Convert a vector of tof bin indices to m/z using this calibration.
		std::vector<double> IndexToMz(const std::vector<size_t>& indices);

		/// @brief  Fill a pre-allocated buffer with m/z (mass to charge) values corresponding to the TOF bin indices int the input.
		/// @param indices TOF bin indices for which calibrated m/z values are desired.
		/// @param mzOut The output buffer.  This will be adjusted by a resize() operation to match the length of "indices"
		void IndexToMzBuffer(const std::vector<size_t>& indices, std::vector<double>& mzOut);
	...
	*   @brief An object that that holds coefficients to convert to/from AT to CCS
	    [[deprecated]]
	*/
